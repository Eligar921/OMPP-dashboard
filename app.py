...
if df_main_filtered is not None and 'Город' in df_main_filtered.columns:
    ...
    if df_kpi_filtered is not None and 'Город' in df_kpi_filtered.columns:
        # Исключаем пустые рекрутеры и источники (уже отфильтровано по источнику, но проверим рекрутера)
        df_kpi_city = df_kpi_filtered[
            df_kpi_filtered['Рекрутер'].notna() & 
            (df_kpi_filtered['Рекрутер'].astype(str).str.strip() != '') &
            df_kpi_filtered['Город'].notna() & 
            (df_kpi_filtered['Город'].astype(str).str.strip() != '')
        ].copy()
        df_kpi_city['Город'] = df_kpi_city['Город'].astype(str).str.strip()
...
# ---- Удаляем старые отдельные блоки по городам (5 и 6 и 9) ----
# Теперь они заменены этим объединённым блоком.
