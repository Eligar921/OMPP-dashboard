...
if df_main_filtered is not None:
    ...
    if not available_sources:
        ...
        # Добавляем итоговую строку (по выбранным рекрутерам)
        total_detail = detail.groupby('Источник ОМПП')['Кол-во'].sum().reset_index()
        total_detail['Рекрутер'] = 'Итого (по выбранным)'
        total_detail['Всего_рекрутер'] = grand_total
        total_detail['% от рекрутера'] = '100%'
        total_detail['% от всех'] = '100%'
        detail = pd.concat([detail, total_detail], ignore_index=True)

        # Если есть данные по всем рекрутерам, добавим строку "Итого (включая скрытых)"
        if df_main_filtered_all is not None:
            detail_all = df_main_filtered_all.groupby(['Рекрутер', 'Источник ОМПП'])['Телефон'].nunique().reset_index()
            detail_all.columns = ['Рекрутер', 'Источник ОМПП', 'Кол-во']
            total_all = detail_all.groupby('Источник ОМПП')['Кол-во'].sum().reset_index()
            total_all['Рекрутер'] = 'Итого (включая скрытых)'
            total_all['Всего_рекрутер'] = total_all['Кол-во'].sum()
            total_all['% от рекрутера'] = '100%'
            total_all['% от всех'] = '100%'
            detail = pd.concat([detail, total_all], ignore_index=True)
...
project_data_all = get_project_data(df_main_filtered_all, df_kpi_filtered_all)

if not project_data.empty and project_data['Кол-во приглашенных'].sum() > 0:
    # Добавляем итоговые строки
    total_row = {'Проект': 'Итого (по выбранным)'}
    avg_row = {'Проект': 'Среднее (по выбранным)'}
    for col in ['Кол-во приглашенных', 'Кол-во вышедших (из приглашенных)', 'Кол-во вышедших (с дошедшими)']:
        if col in project_data.columns:
            total_row[col] = project_data[col].sum()
            avg_row[col] = round(project_data[col].mean(), 1)
    
    # Конверсии от итогов
    total_invited = total_row.get('Кол-во приглашенных', 0)
    total_worked_inv = total_row.get('Кол-во вышедших (из приглашенных)', 0)
    total_worked_done = total_row.get('Кол-во вышедших (с дошедшими)', 0)
    total_row['Конв. из приглашенных в вышедших из пригл., %'] = (
        f"{(total_worked_inv / total_invited * 100).round(1)}%" if total_invited > 0 else "0%"
    )
    total_row['Конв. из приглашенных в вышедших с дошедшими, %'] = (
        f"{(total_worked_done / total_invited * 100).round(1)}%" if total_invited > 0 else "0%"
    )
    
    # Средние по строкам
    for col in ['Конв. из приглашенных в вышедших из пригл., %', 'Конв. из приглашенных в вышедших с дошедшими, %']:
        if col in project_data.columns:
            vals = project_data[col].str.replace('%', '').astype(float)
            avg_row[col] = f"{round(vals.mean(), 1)}%"
    
    project_data = pd.concat([project_data, pd.DataFrame([total_row]), pd.DataFrame([avg_row])], ignore_index=True)
    
    # Добавляем "Итого (включая скрытых)"
    if not project_data_all.empty:
        total_all_row = {'Проект': 'Итого (включая скрытых)'}
        for col in ['Кол-во приглашенных', 'Кол-во вышедших (из приглашенных)', 'Кол-во вышедших (с дошедшими)']:
            if col in project_data_all.columns:
                total_all_row[col] = project_data_all[col].sum()
        total_invited_all = total_all_row.get('Кол-во приглашенных', 0)
        total_worked_inv_all = total_all_row.get('Кол-во вышедших (из приглашенных)', 0)
        total_worked_done_all = total_all_row.get('Кол-во вышедших (с дошедшими)', 0)
        total_all_row['Конв. из приглашенных в вышедших из пригл., %'] = (
            f"{(total_worked_inv_all / total_invited_all * 100).round(1)}%" if total_invited_all > 0 else "0%"
        )
        total_all_row['Конв. из приглашенных в вышедших с дошедшими, %'] = (
            f"{(total_worked_done_all / total_invited_all * 100).round(1)}%" if total_invited_all > 0 else "0%"
        )
        project_data = pd.concat([project_data, pd.DataFrame([total_all_row])], ignore_index=True)
    
    project_data = project_data.sort_values('Кол-во приглашенных', ascending=False)
...
if df_kpi_filtered is not None:
    ...
    if not available_sources_kpi:
        ...
        # Добавляем итоговые строки
        total_kpi = detail_kpi.groupby('Источник ОМПП')['Кол-во'].sum().reset_index()
        total_kpi['Рекрутер'] = 'Итого (по выбранным)'
        total_kpi['Всего_рекрутер'] = grand_total_kpi
        total_kpi['% от рекрутера'] = '100%'
        total_kpi['% от всех'] = '100%'
        detail_kpi = pd.concat([detail_kpi, total_kpi], ignore_index=True)

        if df_kpi_filtered_all is not None:
            detail_kpi_all = df_kpi_filtered_all.groupby(['Рекрутер', 'Источник ОМПП'])['Телефон гигера'].nunique().reset_index()
            detail_kpi_all.columns = ['Рекрутер', 'Источник ОМПП', 'Кол-во']
            total_kpi_all = detail_kpi_all.groupby('Источник ОМПП')['Кол-во'].sum().reset_index()
            total_kpi_all['Рекрутер'] = 'Итого (включая скрытых)'
            total_kpi_all['Всего_рекрутер'] = total_kpi_all['Кол-во'].sum()
            total_kpi_all['% от рекрутера'] = '100%'
            total_kpi_all['% от всех'] = '100%'
            detail_kpi = pd.concat([detail_kpi, total_kpi_all], ignore_index=True)
